...
class CategoryManager(django.db.models.Manager):
    def filter_by_substring(self, substring):
        return (
            self.get_queryset()
            .filter(name__icontains=substring)
        )
...
class QuestionAnswerManager(django.db.models.Manager):
    def filter_by_test_id_or_user_id(self, test_id=None, user_id=None):
        if test_id and user_id:
            return (
                self.get_queryset()
                .filter(question__test__id=test_id, user__id=user_id)
            )
        if test_id:
            return (
                self.get_queryset()
                .filter(question__test__id=test_id)
            )
        if user_id:
            return (
                self.get_queryset()
